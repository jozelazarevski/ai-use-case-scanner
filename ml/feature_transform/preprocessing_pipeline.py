...
class FeatureTransformer:
    ...
    def _build_pipeline(self, df):
        ...
        cat_cols = [col for col in df.columns if not pd.api.types.is_numeric_dtype(df[col]) 
                   and df[col].nunique() <= self.categorical_threshold]
    ...
    def transform(self, df, return_merged=True):
        ...
        if self.pipeline is not None:
            ...
            # If return_merged, merge with the original data
            if return_merged:
                # Get the column names for the transformed data
                transformed_cols = self.generated_columns
                ...
                return pd.concat([input_df, transformed_df], axis=1)
            else:
...
class FeatureGeneratorTransformer(BaseEstimator, TransformerMixin):
    ...
    def _identify_features_to_generate(self, df):
        ...
        for col in df.columns:
            if pd.api.types.is_numeric_dtype(df[col]):
                if not self.target_only:
                    # Standard features for all numeric columns
                    features.extend([
                        f"{col}_zscore",
                        f"{col}_is_outlier",
                        f"{col}_is_high",
                ...
                # Categorical features (suitable as targets) for all numeric columns
                if df[col].nunique() > 3:
                    features.extend([
                        f"{col}_category",
                        f"{col}_trend"
